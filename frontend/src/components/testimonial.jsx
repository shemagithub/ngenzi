...
const Testimonials = () => {
  ...
  // Fetch testimonials from API (similar to admin panel pattern)
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        const apiUrl = `${Backendurl}/api/testimonials/list`;
        ...
        if (response.data.success && response.data.testimonials) {
          ...
          if (testimonialsArray.length === 0) {
            console.warn('No active testimonials found in database');
            setTestimonials([]);
            setError(null); // Don't show error, just empty state
            return;
          }

          // Backend already handles image URL conversion and sorting
          // Map to frontend format while preserving all backend data
        ...
        setError(err.response?.data?.message || err.message || 'Failed to fetch testimonials. Please try again later.');
        setTestimonials([]);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <section className="relative py-32 bg-gradient-to-br from-slate-50 via-blue-50/50 to-indigo-50 dark:from-gray-900 dark:via-gray-800/50 dark:to-gray-900 overflow-hidden transition-colors duration-200">
    ...
    return (
      <section className="relative py-32 bg-gradient-to-br from-slate-50 via-blue-50/50 to-indigo-50 dark:from-gray-900 dark:via-gray-800/50 dark:to-gray-900 overflow-hidden transition-colors duration-200">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-4xl font-bold mb-4 text-gray-900 dark:text-gray-100">Client Testimonials</h2>
            <p className="text-gray-600 dark:text-gray-400">{error || "Testimonials will be displayed here soon."}</p>
          </motion.div>
        </div>
      </section>
    );
