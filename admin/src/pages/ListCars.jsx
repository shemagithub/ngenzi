...
const CarListings = () => {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="admin-page">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div variants={itemVariants} className="mb-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-100 rounded-lg">
                  <Car className="w-6 h-6 text-haven-700" />
                </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/add-cars" className="flex items-center gap-2 px-5 py-2.5 bg-haven-800 text-white rounded-xl hover:bg-haven-700 transition-colors font-medium shadow-lg shadow-blue-500/25">
        ...
        <motion.div variants={itemVariants} className="mb-6 flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search by brand, model, location..." value={searchTerm}
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-haven-600 focus:border-transparent" />
          </div>
          <select value={filterAvailability} onChange={(e) => setFilterAvailability(e.target.value)}
            className="px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-haven-600">
        ...
        {filteredCars.length === 0 ? (
          <motion.div variants={itemVariants} className="text-center py-16 bg-white rounded-2xl border border-gray-200">
            ...
            <Link to="/add-cars" className="inline-flex items-center gap-2 px-5 py-2.5 bg-haven-800 text-white rounded-xl hover:bg-haven-700">
              ...
              {filteredCars.map((car) => (
                <motion.div key={car.id} variants={itemVariants} layout
                  ...
                  <div className="relative h-48 bg-gray-100">
                    ...
                    <span className={`absolute top-3 left-3 px-2.5 py-1 rounded-full text-xs font-semibold ${car.availability === "buy" ? "bg-green-500 text-white" : "bg-haven-700 text-white"}`}>
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <p className="text-2xl font-bold text-haven-700">{formatPrice(car.price)}</p>
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-4 border-t border-gray-200">
                      ...
                      <Link to={`/update-car/${car.id}`} className="flex items-center justify-center gap-1.5 px-2 py-2 bg-haven-800 text-white rounded-lg hover:bg-haven-700 text-sm font-medium">
